...
const activities = [
  ...
  { role: 'Registration Committee Member', organization: "Techxetra '24, Tezpur University", duration: 'Nov 2024', description: 'Managed event registrations and coordinated participant queries during the Techxetra festival.', icon: <FaUserGraduate className="text-xl text-blue-600 dark:text-blue-400" />, iconBg: 'bg-blue-100 dark:bg-blue-900/40', accent: 'hover:border-blue-400 dark:hover:border-blue-500', bar: 'from-blue-500 to-blue-300' },
  { role: 'Member', organization: 'Google Developer Groups, Tezpur University', duration: 'Aug 2023 – June 2024', description: 'Participated in GDG meetups and contributed to the tech community.', icon: <FaGithub className="text-xl text-gray-700 dark:text-gray-300" />, iconBg: 'bg-gray-100 dark:bg-gray-800/60', accent: 'hover:border-gray-400 dark:hover:border-gray-500', bar: 'from-gray-500 to-gray-300' },
  { role: 'Student Volunteer', organization: 'National Service Scheme (NSS)', duration: 'Nov 2022 – June 2024', description: 'Assisted in community service programs, awareness campaigns, and social initiatives.', icon: <FaHandsHelping className="text-xl text-emerald-600 dark:text-emerald-400" />, iconBg: 'bg-emerald-100 dark:bg-emerald-900/40', accent: 'hover:border-emerald-400 dark:hover:border-emerald-500', bar: 'from-emerald-500 to-emerald-300' },
]
...
const Card = ({ children, accent, bar }) => (
  <motion.div
    whileHover={{ y: -4 }}
    className={`light-card glow-card group rounded-2xl p-5 flex flex-col gap-3
...
const Activities = () => {
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-64 h-64 bg-violet-300/12 dark:bg-fuchsia-500/8 blur-[90px] rounded-full pointer-events-none -z-10" />
      ...
      {/* Activities */}
      <div className="flex items-center gap-3 mb-5">
        <span className="text-base font-bold text-gray-700 dark:text-white whitespace-nowrap">Extracurricular</span>
          ...
          >
            <Card accent={item.accent} bar={item.bar}>
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs mt-0.5 mb-2">{item.duration}</p>
                <p className="text-gray-600 dark:text-white/70 text-xs sm:text-sm leading-relaxed">{item.description}</p>
              </div>
            </Card>
          </motion.div>
        ))}
      ...
      {/* Certificates */}
      <div className="flex items-center gap-3 mb-5">
        <span className="text-base font-bold text-gray-700 dark:text-white whitespace-nowrap">Certifications</span>
        <div className="flex-1 h-px bg-gradient-to-r from-pink-200 to-transparent dark:from-pink-800/40" />
      </div>
        ...
        {certificates.map((cert, i) => (
          ...
          >
            <Card accent={cert.accent} bar={cert.bar}>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${cert.iconBg}`}>
                {cert.icon}
              </div>
              <div>
                <h3 className="text-sm font-bold text-gray-800 dark:text-white leading-snug">{cert.title}</h3>
                ...
                <div className="flex flex-col gap-1.5">
                  {Object.keys(cert.links).map((key, j) => (
                    <a key={j} href={cert.links[key]} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-violet-600 dark:text-pink-400 hover:text-pink-500 transition-colors">
                      <span className="w-1.5 h-1.5 rounded-full bg-current flex-shrink-0" />
